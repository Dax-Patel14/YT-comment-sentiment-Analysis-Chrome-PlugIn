...
@app.route('/')
def home():
    return "Welcome to the Influencer Insights Flask API"
...
def predict():
    ...
    try:
        ...
        tfidf_matrix = vectorizer.transform(preprocessed_comments)
        feature_names = vectorizer.get_feature_names_out()
        tfidf_df = pd.DataFrame(tfidf_matrix.toarray(), columns=feature_names)

        predictions = model.predict(tfidf_df).tolist()
...
def generate_trend_graph():
    try:
        ...
        for val in [-1, 0, 1]:
            plt.plot(monthly_percentages.index, monthly_percentages[val], marker='o', label=sentiment_labels[val], color=colors[val])
